#!/bin/bash
# setup_all_interactive.sh
# 새로 포맷된 PC에서 실행되는 통합 설치 스크립트 (인터랙티브 모드)
# 이미 설치된 항목에 대해서는 패스할지, 삭제 후 재설치할지 물어봅니다.
# 각 단계에서 실패한 항목은 마지막에 요약해 출력합니다.
...
# 이미 설치된 항목에 대해 재설치 여부를 묻는 함수
prompt_action() {
    local prompt_msg="$1"
    local answer
    read -p "$prompt_msg [S]kip, [R]einstall? " answer
    case "$answer" in
        [Rr]* ) return 0 ;;  # 재설치 선택
        * ) return 1 ;;      # 건너뛰기 선택
    esac
...
install_package() {
    ...
    if dpkg -l | grep -qw "$pkg"; then
        if prompt_action "패키지 '$pkg'가 이미 설치되어 있습니다. 삭제 후 재설치하시겠습니까?"; then
            echo "패키지 '$pkg' 제거 후 재설치 중..."
            apt-get remove --purge -y "$pkg" && apt-get install -y "$pkg" || log_failure "패키지 '$pkg' 재설치 실패"
        else
            echo "패키지 '$pkg' 설치 건너뛰기"
        fi
    else
        apt-get install -y "$pkg" || log_failure "패키지 '$pkg' 설치 실패"
    fi
}
...
##########################
# Git 리포지토리 클론 함수 (디렉토리 존재 시 재설치 여부 질문)
##########################
clone_repo() {
    local repo_url="$1"
    local target_dir="$2"
    if [ -d "$target_dir" ]; then
        if prompt_action "디렉토리 '$target_dir'가 이미 존재합니다. 제거 후 다시 클론하시겠습니까?"; then
            rm -rf "$target_dir" || log_failure "디렉토리 '$target_dir' 제거 실패"
            git clone "$repo_url" "$target_dir" || log_failure "리포지토리 '$repo_url' 클론 실패"
        else
            echo "리포지토리 '$repo_url' 클론 건너뛰기"
        fi
    else
        git clone "$repo_url" "$target_dir" || log_failure "리포지토리 '$repo_url' 클론 실패"
    fi
...
##########################
# DKMS 모듈 설치 함수 (모듈이 이미 존재하면 삭제 여부를 질문)
##########################
setup_dkms_module() {
    local module_dir="$1"
    local module_name="$2"
    ...
    if [ -d "/usr/src/${module_name}-${module_version}" ]; then
        if prompt_action "DKMS 모듈 '/usr/src/${module_name}-${module_version}'가 이미 존재합니다. 제거 후 재설치하시겠습니까?"; then
            dkms remove -m "$module_name" -v "$module_version" --all || log_failure "DKMS 모듈 제거 실패: $module_name"
        else
            echo "DKMS 모듈 재설치 건너뛰기"
            return 0
        fi
    fi
    cp -r "$module_dir" "/usr/src/${module_name}-${module_version}" || log_failure "$module_name 소스 복사 실패"
    dkms add -m "$module_name" -v "$module_version" || log_failure "DKMS 추가 실패 ($module_name)"
    dkms build -m "$module_name" -v "$module_version" || log_failure "DKMS 빌드 실패 ($module_name)"
    dkms install -m "$module_name" -v "$module_version" || log_failure "DKMS 설치 실패 ($module_name)"
    modprobe 8812au || log_failure "$module_name 모듈 로드 실패"
}
...
apt-get update || log_failure "패키지 목록 업데이트 실패"
apt-get upgrade -y || log_failure "패키지 업그레이드 실패"

# qt 관련 이전 스크립트에서 사용한 패키지 목록 (필요에 따라 추가/수정)
packages=(
  "build-essential"
  "make"
  "qt5-default"
...
echo "2. 시스템 환경 설정을 진행합니다..."
{
  ...
  source /etc/profile
  ldconfig
} || log_failure "LD_LIBRARY_PATH 추가 또는 ldconfig 실행 실패"
...
# rtl8812au 리포지토리 클론 (대상 디렉토리는 'rtl8812au')
clone_repo "https://github.com/gnab/rtl8812au.git" "rtl8812au"
# DKMS 모듈 설치 (모듈 이름: rtl8812au, 버전: 4.2.2)
...
if [ -d "$CMAKE_DIR" ]; then
    if prompt_action "CMake 디렉토리 '$CMAKE_DIR'가 이미 존재합니다. 제거 후 다시 설치하시겠습니까?"; then
        rm -rf "$CMAKE_DIR" || log_failure "CMake 디렉토리 제거 실패"
    else
        echo "CMake 설치 건너뛰기"
    fi
fi
wget "https://github.com/Kitware/CMake/releases/download/v$CMAKE_VERSION/cmake-$CMAKE_VERSION.tar.gz" -O "$HOME/cmake-$CMAKE_VERSION.tar.gz" || log_failure "CMake 다운로드 실패"
tar -xvzf "$HOME/cmake-$CMAKE_VERSION.tar.gz" -C "$HOME" || log_failure "CMake 압축 해제 실패"
...
if [ -d "$HOME/Sophus" ]; then
    if prompt_action "Sophus 디렉토리 '$HOME/Sophus'가 이미 존재합니다. 제거 후 재클론하시겠습니까?"; then
        rm -rf "$HOME/Sophus" || log_failure "Sophus 디렉토리 제거 실패"
        git clone https://github.com/strasdat/Sophus.git "$HOME/Sophus" || log_failure "Sophus 클론 실패"
    else
        echo "Sophus 클론 건너뛰기"
    fi
else
...
if [ -d "$HOME/gtsam" ]; then
    if prompt_action "GTSAM 디렉토리 '$HOME/gtsam'가 이미 존재합니다. 제거 후 재클론하시겠습니까?"; then
        rm -rf "$HOME/gtsam" || log_failure "GTSAM 디렉토리 제거 실패"
        git clone https://github.com/borglab/gtsam.git "$HOME/gtsam" || log_failure "GTSAM 클론 실패"
    else
        echo "GTSAM 클론 건너뛰기"
    fi
else
...
if [ -d "$HOME/ompl" ]; then
    if prompt_action "OMPL 디렉토리 '$HOME/ompl'가 이미 존재합니다. 제거 후 재클론하시겠습니까?"; then
        rm -rf "$HOME/ompl" || log_failure "OMPL 디렉토리 제거 실패"
        git clone https://github.com/ompl/ompl.git "$HOME/ompl" || log_failure "OMPL 클론 실패"
    else
        echo "OMPL 클론 건너뛰기"
    fi
else
...
if [ -d "$HOME/socket.io-client-cpp" ]; then
    if prompt_action "socket.io-client-cpp 디렉토리 '$HOME/socket.io-client-cpp'가 이미 존재합니다. 제거 후 재클론하시겠습니까?"; then
        rm -rf "$HOME/socket.io-client-cpp" || log_failure "socket.io-client-cpp 디렉토리 제거 실패"
        git clone --recurse-submodules https://github.com/socketio/socket.io-client-cpp.git "$HOME/socket.io-client-cpp" || log_failure "socket.io-client-cpp 클론 실패"
    else
        echo "socket.io-client-cpp 클론 건너뛰기"
    fi
else
    git clone --recurse-submodules https://github.com/socketio/socket.io-client-cpp.git "$HOME/socket.io-client-cpp" || log_failure "socket.io-client-cpp 클론 실패"
fi
...
if [ -d "$HOME/octomap" ]; then
    if prompt_action "OctoMap 디렉토리 '$HOME/octomap'가 이미 존재합니다. 제거 후 재클론하시겠습니까?"; then
        rm -rf "$HOME/octomap" || log_failure "OctoMap 디렉토리 제거 실패"
        git clone https://github.com/OctoMap/octomap.git "$HOME/octomap" || log_failure "OctoMap 클론 실패"
    else
        echo "OctoMap 클론 건너뛰기"
    fi
else
    git clone https://github.com/OctoMap/octomap.git "$HOME/octomap" || log_failure "OctoMap 클론 실패"
fi
...
if [ -d "$HOME/OrbbecSDK" ]; then
    if prompt_action "OrbbecSDK 디렉토리 '$HOME/OrbbecSDK'가 이미 존재합니다. 제거 후 재클론하시겠습니까?"; then
        rm -rf "$HOME/OrbbecSDK" || log_failure "OrbbecSDK 디렉토리 제거 실패"
        git clone https://github.com/orbbec/OrbbecSDK.git "$HOME/OrbbecSDK" || log_failure "OrbbecSDK 클론 실패"
    else
        echo "OrbbecSDK 클론 건너뛰기"
    fi
else
    git clone https://github.com/orbbec/OrbbecSDK.git "$HOME/OrbbecSDK" || log_failure "OrbbecSDK 클론 실패"
fi
...
if [ -d "$HOME/rplidar_sdk" ]; then
    if prompt_action "RPlidar SDK 디렉토리 '$HOME/rplidar_sdk'가 이미 존재합니다. 제거 후 재클론하시겠습니까?"; then
        rm -rf "$HOME/rplidar_sdk" || log_failure "RPlidar SDK 디렉토리 제거 실패"
        git clone https://github.com/Slamtec/rplidar_sdk.git "$HOME/rplidar_sdk" || log_failure "RPlidar SDK 클론 실패"
    else
        echo "RPlidar SDK 클론 건너뛰기"
    fi
else
    git clone https://github.com/Slamtec/rplidar_sdk.git "$HOME/rplidar_sdk" || log_failure "RPlidar SDK 클론 실패"
fi
