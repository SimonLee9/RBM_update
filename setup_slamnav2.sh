...
if [ ! -d "cmake-$CMAKE_VERSION" ]; then
    wget https://github.com/Kitware/CMake/releases/download/v$CMAKE_VERSION/cmake-$CMAKE_VERSION.tar.gz || error_exit "CMake를 다운로드하지 못했습니다."
    tar -xvzf cmake-$CMAKE_VERSION.tar.gz
fi
cd cmake-$CMAKE_VERSION || error_exit "CMake 디렉토리에 들어가지 못했습니다."
./bootstrap --qt-gui || error_exit "CMake 부트스트랩에 실패했습니다."
make -j$NUM_CORES || error_exit "CMake 빌드에 실패했습니다."
sudo make install || error_exit "CMake 설치에 실패했습니다."
cmake --version

# Sophus 설치
echo "Sophus를 설치 중입니다..."
cd ~ || error_exit "홈 디렉토리로 이동하지 못했습니다."
if [ ! -d "Sophus" ]; then
    git clone https://github.com/strasdat/Sophus.git || error_exit "Sophus 리포지토리를 클론하지 못했습니다."
fi
cd Sophus || error_exit "Sophus 디렉토리에 들어가지 못했습니다."
mkdir -p build && cd build
cmake .. -DBUILD_TESTS=OFF -DBUILD_EXAMPLES=OFF -DSOPHUS_USE_BASIC_LOGGING=ON || error_exit "Sophus의 CMake 구성에 실패했습니다."
make -j$NUM_CORES || error_exit "Sophus 빌드에 실패했습니다."
sudo make install || error_exit "Sophus 설치에 실패했습니다."

# GTSAM 설치
echo "GTSAM을 설치 중입니다..."
cd ~ || error_exit "홈 디렉토리로 이동하지 못했습니다."
if [ ! -d "gtsam" ]; then
    git clone https://github.com/borglab/gtsam.git || error_exit "GTSAM 리포지토리를 클론하지 못했습니다."
fi
cd gtsam || error_exit "GTSAM 디렉토리에 들어가지 못했습니다."
git checkout 4.2.0 || error_exit "GTSAM 버전 4.2.0 체크아웃에 실패했습니다."
mkdir -p build && cd build
cmake .. -DGTSAM_USE_SYSTEM_EIGEN=ON -DGTSAM_BUILD_TESTS=OFF -DGTSAM_BUILD_EXAMPLES_ALWAYS=OFF || error_exit "GTSAM의 CMake 구성에 실패했습니다."
make -j$NUM_CORES || error_exit "GTSAM 빌드에 실패했습니다."
sudo make install || error_exit "GTSAM 설치에 실패했습니다."
